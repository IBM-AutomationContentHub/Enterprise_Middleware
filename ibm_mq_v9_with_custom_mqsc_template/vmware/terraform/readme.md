...
Install and configure IBM WebSphere MQSeries on a single Linux Node.<br>This template will install a standalone instance of WebSphere MQSeries<br>,configure a single Queue Manager and run a 'bring your own script', which<br>will execute custom runmqsc commands. Queue Manager details may be<br>configured through CAM Variables or exposed to the deployer to modify<br>on a per-deployment basis.<br>
